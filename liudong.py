...
class FlowingRedFlagEvaluationSystem:
    ...
    def create_sidebar(self):
        ...
        buttons_row1 = [
            ("📊 计算总分", self.calculate_totals),
            ("📋 输出表格", self.calculate_and_output_table)
        ]
        self.create_button_row(self.action_frame, buttons_row1)
        
        buttons_row2 = [
            ("⚖️ 奖惩管理", self.manage_punishments)
        ]
        self.create_button_row(self.action_frame, buttons_row2)
        ...
        self.create_button_row(self.action_frame, buttons_row3)
    ...
    def reset_data(self):
        ...
        self.punishments.clear()
        if self.punishment_list_tree is not None:
    ...
    def manage_punishments(self):
        ...
        # 如果class_combobox已经存在，更新其值；否则创建新的
        if self.class_combobox is not None:
            self.class_combobox['values'] = self.classes
        else:
            self.class_combobox = ttk.Combobox(input_frame, values=self.classes)
            self.class_combobox.grid(row=0, column=1, padx=10, pady=10, sticky="ew")
        
        ttk.Label(input_frame, text="奖惩类型:").grid(row=1, column=0, padx=10, pady=10, sticky="e")
        self.punishment_type = tk.StringVar()
        type_frame = ttk.Frame(input_frame)
        type_frame.grid(row=1, column=1, padx=10, pady=10, sticky="w")
        ttk.Radiobutton(type_frame, text="奖励（加分）", variable=self.punishment_type, value="add").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(type_frame, text="惩罚（减分）", variable=self.punishment_type, value="subtract").pack(side=tk.LEFT, padx=5)
        ...
        self.note_entry.grid(row=3, column=1, padx=10, pady=10, sticky="ew")
        
        button_frame = ttk.Frame(input_frame)
        button_frame.grid(row=4, column=0, columnspan=2, pady=10)
        add_button = ttk.Button(button_frame, text="添加奖惩", command=self.add_punishment)
        add_button.pack(side=tk.LEFT, padx=5)
        delete_button = ttk.Button(button_frame, text="删除奖惩", command=self.delete_punishment)
        ...
        columns = ("班级", "类型", "分值", "备注")
        self.punishment_list_tree = ttk.Treeview(list_frame, columns=columns, show="headings")
        for col in columns:
            self.punishment_list_tree.heading(col, text=col)
            self.punishment_list_tree.column(col, width=150)
        self.punishment_list_tree.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        scrollbar = ttk.Scrollbar(self.punishment_list_tree)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.punishment_list_tree.configure(yscrollcommand=scrollbar.set)
        scrollbar.configure(command=self.punishment_list_tree.yview)
        
        for cls, punishments in self.punishments.items():
            for punishment in punishments:
                type_text = "奖励" if punishment["type"] == "add" else "惩罚"
                self.punishment_list_tree.insert("", "end", values=(cls, type_text, punishment["score"], punishment["note"]))

    def add_punishment(self):
        cls = self.class_combobox.get()
        if not cls:
            messagebox.showerror("错误", "请选择班级！")
            return
        ptype = self.punishment_type.get()
        if not ptype:
        ...
        self.punishment_list_tree.insert("", "end", values=(cls, ptype, score, note))
    ...
    def delete_punishment(self):
        selected_item = self.punishment_list_tree.selection()
        ...
        values = self.punishment_list_tree.item(item, "values")
        cls = values[0]
        ptype = values[1]
    ...
    def add_class(self):
        ...
        if new_class_name in existing_classes:
            counter = 1
            while f"{new_class_name}{counter}" in existing_classes:
                counter += 1
            new_class_name = f"{new_class_name}{counter}"
    ...
    def show_about(self):
        ...
        ttk.Label(dev_frame, text="开发团队: 王腾鹤", font=("微软雅黑", 10)).pack(anchor=tk.W)
    ...
    def load_data(self):
        try:
            ...
            self.load_scores(data.get('scores', {}))
    ...
    def show_history(self):
        ...
        def load_selected():
            ...
            if 0 <= index < len(history):
                ...
                messagebox.showerror("错误", "无法加载选中的记录")
